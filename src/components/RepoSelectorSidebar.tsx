...
export default function RepoSelectorSidebar() {
  ...
  return (
    ...
    >
      <div
        className={`flex items-center shrink-0 border-b border-gray-200 dark:border-white/[0.08] gap-2 ${
          expanded ? 'justify-between p-4' : 'justify-center p-2'
        }`}
      >
        {expanded && (
          <h2 className="m-0 text-xs font-semibold uppercase tracking-wider text-gray-500 dark:text-white/60 flex-1 min-w-0">
            Repositories
          </h2>
        )}
        <button
          type="button"
          className="shrink-0 w-7 h-7 flex items-center justify-center rounded-md bg-gray-200 dark:bg-white/10 border border-gray-300 dark:border-white/20 text-gray-700 dark:text-white/80 cursor-pointer text-xs hover:bg-gray-300 dark:hover:bg-white/20"
            ...
            <ul className="list-none m-0 p-0">
              {availableRepos.map((repo) => (
                <li key={repo.id} className="m-0">
                  <label className="flex items-center gap-2 px-3 py-2 cursor-pointer rounded-md text-sm text-gray-800 dark:text-inherit hover:bg-gray-200 dark:hover:bg-white/5">
                    <input
                      type="checkbox"
      ...
      {expanded && selectedRepos.length > 0 && (
        <div className="px-4 py-3 text-xs text-gray-500 dark:text-white/50 border-t border-gray-200 dark:border-white/[0.08]">
